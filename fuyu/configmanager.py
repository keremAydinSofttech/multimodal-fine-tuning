...
class ConfigurationManager:
    ...
    def __init__(self, config_file_path='./train_config_file.yaml'):

        if os.path.exists(config_file_path):
            print(f"{config_file_path} dosyası mevcut.")
        else:
            print(f"{config_file_path} dosyası bulunamadı.")
        self.config_readable, self.config_changeable = self.read_config_file(config_file_path)
    ...
    def read_config_file(self, config_file_path):
        ...
        try:
            path_of_the_config_yaml = config_file_path
            ...
            path_of_the_config_yaml = './open_llm/starcoder/for_cloud/config_file.yaml'
            config_f1 =configparser.ConfigParser()
            config_f1.read(path_of_the_config_yaml)
            config_readable = config_f1['readable']
            config_changeable = config_f1['changeable']
