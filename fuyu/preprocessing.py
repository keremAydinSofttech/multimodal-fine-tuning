...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        text_prompt = self.list_data_dict[i]['text_prompt']
        ...
        if 'image_id' in sources[0]:
            ...
            bus_image_pil = Image.open(os.path.join(image_folder, image_file)).convert('RGB')
            inputs_to_model = processor(text=text_prompt, images=bus_image_pil)     

        else:
            tokenizer = self.data_args.tokenizer
            inputs_to_model = tokenizer(text=text_prompt, return_tensors="pt")

            
        data_dict = {'input_ids': inputs_to_model}
